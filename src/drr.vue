<template>
	<div class="drr" :style="style" :class="classObject" @dblclick="dblclick($event)" @mousedown="bodyMouseDown($event)"
		@touchstart.stop.prevent="bodyMouseDown($event)" v-touch:start="deselect" v-touch:end="up">
		<slot>
		</slot>
		<div v-for="stick in sticks" class="drr-stick" :class="['drr-stick-' + stick, resizable ? '' : 'not-resizable']"
...
export default {
	...
	methods: {
		...
		bodyMouseDown: function (e) {
			...
			this.startRect = this.cloneDeep(this.getRect())
